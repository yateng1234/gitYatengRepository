  ...
  d2-container
    .mod-menu
      el-form(:inline='true', :model='dataForm')
        el-form-item
          el-button(v-if="isAuth('sys:menu:save')", type='primary', @click='addOrUpdateHandle()') 新增
      el-table(:data='dataList', border='', v-loading='dataListLoading', style='width: 100%;')
        ...
        el-table-column(prop='parentName', header-align='center', align='center', width='120', label='上级菜单')
          template(slot-scope='scope')
            ...
            span(v-else='')  {{ scope.row.parentName }}
        el-table-column(header-align='center', align='center', label='图标')
          template(slot-scope='scope')
            i(:class="'fa fa-' + scope.row.icon")
        el-table-column(prop='type', header-align='center', align='center', label='类型')
          template(slot-scope='scope')
            el-tag(v-if='scope.row.type === 0', size='small') 目录
            el-tag(v-else-if='scope.row.type === 1', size='small', type='success') 菜单
            el-tag(v-else-if='scope.row.type === 2', size='small', type='info') 按钮
        ...
        el-table-column(fixed='right', header-align='center', align='center', width='150', label='操作')
          template(slot-scope='scope')
            el-button(v-if="isAuth('sys:menu:update')", type='text', size='small', @click='addOrUpdateHandle(scope.row.menuId)') 修改
            el-button(v-if="isAuth('sys:menu:delete')", type='text', size='small', @click='deleteHandle(scope.row.menuId)') 删除
